...
# 最終的な出力Dが、D = |Pk - Pj| = Plとして表されることに着目する。
# Pk > Pjであると仮定すると、
...
# {n_k - n_j}{3(n_k + n_j) - 1} = Pl = n_l*(3*n_l-1)
# 
# ここで、n_l*(3*n_l-1)に関して、n_lと(3*n_l-1)は互いに素であり、2つの数の掛け合わせ(ただし、はn_l < (3*n_l-1))で構成されることに着目する。
...
# (6)n_kとn_jを導出したら、それぞれを使用して五角数を出し、足し合わせる。
...
# 
# ※n_lと(3*n_l-1)は互いに素であることの証明
# 背理法による証明のため、n_lと(3*n_l-1)に共通する素因数があると仮定して、
# n_l = g*x, 3*n_l-1 = g*yと置く。(g>1およびn_l>1より、n_l>x, 3*n_l-1>y,y>x)
# この式からgを消去すると、n_l*x = (3*n_l-1)*y
# この式をn_lの式として括ると、(3*y-x)*n_l = y
# n_l,y,xは1以上の整数であるため、3*y-x<=0は等式を満たさなくなるため不適。
# 3*y-x>0においても、n_l=y/(3*y-x)となり、n_lが整数となるためには(3*y-x)<y <=> 2*y<x
# これはy<xの条件と矛盾するため不適。よってどの場合でもn_lと(3*n_l-1)は共通する素因数がないため、仮定が間違っていることがわかる。
# よって、n_lと(3*n_l-1)は互いに素である。
